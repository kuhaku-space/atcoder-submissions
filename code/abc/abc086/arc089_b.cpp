...
int main(void) {
    ...
    vector dp(k + 1, vector(k + 1, 0));
    for (auto [x, y, c] : v) {
        int rx = x % k, ry = y % k, r = (x / k + y / k + (c == 'W')) % 2;
        if (r == 0)
            r = -1;
        dp[rx][ry] += r;
    }
    ...
    auto prod = [&](int xl, int yl, int xr, int yr) {
        return dp[xl][yl] - dp[xl][yr] - dp[xr][yl] + dp[xr][yr];
    };

    int ans = 0;
    rep (x, k) {
        rep (y, k) {
            int s = prod(0, 0, x, y) + prod(x, y, k, k) - prod(0, y, x, k) - prod(x, 0, k, y);
            if (s < 0)
                s = -s;
            int r = (n - s) / 2;
            chmax(ans, n - r);
