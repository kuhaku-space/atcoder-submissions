...
int main(void) {
    ...
    vector dp(n + 3, vector(n * n + 100, Mint()));
    dp[0][0] = 1;
    rep (i, n) {
        vector cp(n + 3, vector(n * n + 100, Mint()));
        rep (j, n) {
            rep (k, n * n) {
                if (dp[j][k] == 0)
                    continue;
                cp[j][k + 2 * (i - j) + 2] += dp[j][k];
                cp[j + 1][k + 2 * (i - j)] += dp[j][k] * (i - j) * 2;
                cp[j + 1][k + 2 * (i - j)] += dp[j][k];
                cp[j + 2][k + 2 * (i - j) - 2] += dp[j][k] * (i - j) * (i - j);
            }
        }
        dp = cp;
    }
