...
Mint PowMod(Mint a, ll n){
	if(n < 0) return PowMod(PowMod(a, -n), MOD-2);
	if(n == 0) return 1;
	if(n == 1) return a;
	if(n%2 == 0) return PowMod(a*a, n/2);
	return a*PowMod(a*a, n/2);
}
...
void init(ll n = 2e6){
	fac.resize(n+1);
	fac[0] = 1;
	repn(i, n) fac[i] = fac[i-1]*i;
	finv.resize(n+1);
	finv[n] = PowMod(fac[n], MOD-2);
	repr(i, n) finv[i] = finv[i+1]*(i+1);
	finv[0] = 1;
}
...
int main(void){
	...
	Mint ans = 0;
	rep(i, k+1){
		ans += PowMod(25, i) * combi(s.size() - 1 + i, i) * PowMod(26, k - i);
	}
	co(ans);
