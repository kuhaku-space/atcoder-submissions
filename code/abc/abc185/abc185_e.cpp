...
ll f(ll x, ll y) {
    if (memo[x][y] != LINF) return memo[x][y];
    if (x == n && y == m) return memo[x][y] = 0;
    if (x == n) return memo[x][y] = f(x, y + 1) + 1;
    if (y == m) return memo[x][y] = f(x + 1, y) + 1;
    if (a[x] == b[y]) return memo[x][y] = f(x + 1, y + 1);

    return memo[x][y] = min({f(x, y + 1) + 1, f(x + 1, y) + 1, f(x + 1, y + 1) + 1});
}

int main(void) {
    rep(i, 1005) rep(j, 1005) memo[i][j] = LINF;
    cin >> n >> m;
    a.resize(n);
    b.resize(m);
    cin >> a >> b;
