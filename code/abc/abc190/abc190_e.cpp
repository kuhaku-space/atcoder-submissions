...
template <class T>
std::vector<T> dijkstra(const Graph<void> &g, int s = 0, T inf = std::numeric_limits<T>::max()) {
    struct _edge {
        int to;
        T dist;
    ...
    while (!p_que.empty()) {
        ...
        if (dist[e.to] < e.dist)
            continue;
        for (auto &i : g[e.to]) {
            if (chmin(dist[i], e.dist + 1))
                p_que.emplace(i, e.dist + 1);
        }
    }
    return dist;
}
...
int main(void) {
    ...
    rep(i, k) gs[i] = dijkstra<int>(g, c[i] - 1, Inf);
    ...
    rep(i, 1 << k) {
        rep(j, k) {
            rep(l, k) {
                if (!(i >> j & 1) || (i >> l & 1))
                    continue;
                chmin(dp[i | 1 << l][l], dp[i][j] + gs[j][c[l] - 1]);
            }
        }
    }
