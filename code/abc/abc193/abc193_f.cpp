...
int main(void) {
    ...
    int s = mf.size() - 2, t = s + 1;
    rep (i, n) {
        rep (j, n) {
            if (b[i][j] == 'B') {
                if ((i + j) & 1)
                    mf.add_edge(s, g.flatten(i, j), Inf);
                else
                    mf.add_edge(g.flatten(i, j), t, Inf);
            }
            if (b[i][j] == 'W') {
                if ((i + j) & 1)
                    mf.add_edge(g.flatten(i, j), t, Inf);
    ...
    rep (i, n) {
        rep (j, n) {
            if (g.in_field(i, j + 1)) {
                mf.add_edge(g.flatten(i, j), g.flatten(i, j + 1), 1);
                mf.add_edge(g.flatten(i, j + 1), g.flatten(i, j), 1);
            }
            if (g.in_field(i + 1, j)) {
                mf.add_edge(g.flatten(i, j), g.flatten(i + 1, j), 1);
                mf.add_edge(g.flatten(i + 1, j), g.flatten(i, j), 1);
            }
        }
    }
    co(n * (n - 1) * 2 - mf.flow(s, t));
    return 0;
}
