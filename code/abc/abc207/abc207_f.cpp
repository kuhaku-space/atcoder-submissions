...
int main(void) {
    ...
    Graph<void> g(n);
    g.input_edges(n - 1);
    vector<vector<vector<Mint>>> dp(3, vector<vector<Mint>>(n, vector<Mint>(2)));
    vector<int> sz(n, 2);
    auto dfs = [&](auto self, int idx, int par) -> void {
        ...
        for (auto e : g[idx]) {
            if (e.to() == par)
                continue;
            self(self, e.to(), idx);
            vector<Mint> cp0(sz[idx] + sz[e.to()]), cp1(sz[idx] + sz[e.to()]),
                cp2(sz[idx] + sz[e.to()]);
            rep (i, sz[idx]) {
                rep (j, sz[e.to()]) {
                    cp0[i + j] += dp[0][idx][i] * (dp[0][e.to()][j] + dp[1][e.to()][j]);
                    cp1[i + j + 1] += dp[0][idx][i] * dp[2][e.to()][j];
                    cp1[i + j] +=
                        dp[1][idx][i] * (dp[0][e.to()][j] + dp[1][e.to()][j] + dp[2][e.to()][j]);
                    cp2[i + j + 1] += dp[2][idx][i] * dp[0][e.to()][j];
                    cp2[i + j] += dp[2][idx][i] * (dp[1][e.to()][j] + dp[2][e.to()][j]);
                }
            }
            sz[idx] += sz[e.to()];
            dp[0][idx] = cp0;
