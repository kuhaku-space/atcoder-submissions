...
int main(void) {
    ...
    rep(i, d + 1) {
        if (i == 0 || i == d) {
            ...
            sum += Mint(2).pow(i - 1) * Mint(2).pow(d - i - 1);
            dp[max(i, d - i)] += Mint(2).pow(i - 1) * Mint(2).pow(d - i - 1);
        }
    }

    Mint ans = 0;
    rep(i, n) {
        while (!dp.empty() && prev(dp.end())->first > n - i - 1) {
            sum -= prev(dp.end())->second;
            dp.erase(prev(dp.end()));
        }
        ans += sum * Mint(2).pow(i + 1);
    }
    co(ans);
