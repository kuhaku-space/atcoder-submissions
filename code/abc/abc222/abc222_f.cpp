...
int main(void) {
    ...
    auto dfs = [&](auto &&self, int idx, int par) -> ll {
        parent[idx] = par;
        v[idx].resize(g[idx].size());
        int cnt = 0;
        for (auto i : g[idx]) {
            if (i.to != par)
                v[idx][cnt] = self(self, i.to, idx) + i.dist;
            ++cnt;
        }
        if (g[idx].size() == 1)
            return d[idx];
        return max(*max_element(all(v[idx])), d[idx]);
    };
    dfs(dfs, 0, -1);
    ...
    auto bfs = [&](int root) {
        ...
        while (!que.empty()) {
            ...
            for (auto i : g[idx]) {
                if (i.to == par)
                    v[idx][cnt] = tmp[idx] + i.dist;
                ++cnt;
            }
            int sz = g[idx].size();
            vector<ll> l(sz), r(sz);
            rep(i, sz - 1) {
                l[i + 1] = max(l[i], v[idx][i]);
            }
            repr(i, sz - 1) {
                r[i] = max(r[i + 1], v[idx][i + 1]);
            }
            vector<ll> a(sz);
            rep(i, sz) a[i] = max(l[i], r[i]);
            cnt = 0;
            for (auto i : g[idx]) {
                if (i.to != par) {
                    tmp[i.to] = max(a[cnt], d[idx]);
                    que.emplace(i.to, idx);
                }
                ++cnt;
            }
        }
    ...
    vector<ll> ans(n);
    rep(i, n) ans[i] = *max_element(all(v[i]));
    rep(i, n) co(ans[i]);
