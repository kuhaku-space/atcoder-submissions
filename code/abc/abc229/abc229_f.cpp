...
#line 2 "/home/kuhaku/kuhaku/github/atcoder-lib/lib/graph/max_flow.hpp"
...
int main(void) {
    ...
    FOR(i, 2, n + 1) {
        rep(j, 2) {
            chmin(dp[i][j][0], dp[i - 1][j][0] + a[i - 1] + b[i - 2]);
            chmin(dp[i][j][0], dp[i - 1][j][1] + a[i - 1]);
            chmin(dp[i][j][1], dp[i - 1][j][0]);
            chmin(dp[i][j][1], dp[i - 1][j][1] + b[i - 2]);
        }
    }
