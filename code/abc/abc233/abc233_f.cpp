...
int main(void) {
    ...
    while (!st.empty()) {
        ...
        while (parents[x] != -1) {
            p.emplace_back(parents[x]);
            auto e = edges[parents[x]];
            x = e.first + e.second - x;
        }
        int y = idx;
        while (parents[y] != -1) {
            q.emplace_back(parents[y]);
            auto e = edges[parents[y]];
            y = e.first + e.second - y;
        }
        reverse(all(q));
        for (auto x : q) {
            if (!p.empty() && p.back() == x) {
        ...
        for (auto x : p) {
            swap(a[edges[x].first], a[edges[x].second]);
        }
