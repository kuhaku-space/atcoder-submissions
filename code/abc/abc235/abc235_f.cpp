...
int main(void) {
    ...
    int n = s.size();
    vector<Mint> p(n);
    rep(i, n) p[i] = Mint(10).pow(n - i - 1);
    vector<vector<Mint>> dp(1 << 10, vector<Mint>(2));
    auto dp2 = dp;
    rep(i, n) {
        vector<vector<Mint>> dp3(1 << 10, vector<Mint>(2));
        vector<vector<Mint>> dp4(1 << 10, vector<Mint>(2));
        rep(j, 10) {
            rep(k, 1 << 10) {
                dp3[k | (1 << j)][0] += dp[k][0] + dp2[k][0] * p[i] * j;
                dp4[k | (1 << j)][0] += dp2[k][0];
                if (j == s[i] - '0') {
                    dp3[k | (1 << j)][1] += dp[k][1] + dp2[k][1] * p[i] * j;
                    dp4[k | (1 << j)][1] += dp2[k][1];
                } else if (j < s[i] - '0') {
                    dp3[k | (1 << j)][0] += dp[k][1] + dp2[k][1] * p[i] * j;
                    dp4[k | (1 << j)][0] += dp2[k][1];
                }
            }
            if (j) {
