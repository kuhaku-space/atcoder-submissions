...
struct F {
    ...
    static constexpr T id = std::numeric_limits<T>::max();
    static constexpr T op(const T &lhs, const T &rhs) {
        return lhs == F::id ? rhs : lhs;
    }

    template <class U>
    static constexpr U f(T lhs, U rhs) {
        return lhs == F::id ? rhs : make_pair(lhs * rhs.second, rhs.second);
    }
};
...
int main(void) {
    ...
    vector<int> a(n);
    cin >> a;
    lazy_segment_tree<M, F> lst1(n), lst2(n);
    rep (i, n) {
        if (a[i] < x)
            lst1.set(i, {0, 1});
        else
    ...
    while (q--) {
        ...
        cin >> c >> l >> r;
        --l;
        auto p = lst1.prod(l, r);
        auto q = lst2.prod(l, r);
        if (c == 1) {
            int x = p.second - p.first;
            lst1.apply(l, l + x, 0);
            lst1.apply(l + x, r, 1);
            int y = q.second - q.first;
            lst2.apply(l, l + y, 0);
            lst2.apply(l + y, r, 1);
        } else {
            int x = p.first;
    ...
    rep (i, n) {
        if (lst1.get(i).first != lst2.get(i).first) {
            co(i + 1);
        }
    }
