...
int main(void) {
    ...
    rep (i, n) {
        ...
        if (m == 0)
            continue;
        mf_graph<int> mf(n * (n - 1) / 2 + n + 2);
        int s = n * (n - 1) / 2 + n, t = s + 1;
        rep (j, n * (n - 1) / 2) mf.add_edge(s, j, 1);
        rep (j, n) {
            if (j == i)
                mf.add_edge(n * (n - 1) / 2 + j, t, m);
            else
                mf.add_edge(n * (n - 1) / 2 + j, t, m - 1);
        }
        ...
        rep (x, n) {
            rep (y, x) {
                if (u[x][y] != -1)
                    mf.add_edge(k, n * (n - 1) / 2 + x, 1);
                if (u[y][x] != -1)
                    mf.add_edge(k, n * (n - 1) / 2 + y, 1);
                ++k;
            }
        }
        if (mf.flow(s, t) == n * (n - 1) / 2)
            ans.emplace_back(i);
    }
    Inc >> ans;
    co(ans);
