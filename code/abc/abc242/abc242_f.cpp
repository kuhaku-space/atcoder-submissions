...
int main(void) {
    int n, m, b, w;
    cin >> n >> m >> b >> w;
    vector dp1(n + 1, vector(m + 1, Mint()));
    vector dp2(n + 1, vector(m + 1, Mint()));
    repn (i, n) {
        repn (j, m) {
            dp1[i][j] = combi(i * j, b);
            dp2[i][j] = combi(i * j, w);
        ...
        repn (j, m) {
            rep (k, i + 1) {
                rep (l, j + 1) {
                    if (i == k && j == l)
                        continue;
                    dp1[i][j] -= combi(i, k) * combi(j, l) * dp1[k][l];
                    dp2[i][j] -= combi(i, k) * combi(j, l) * dp2[k][l];
                }
            }
        }
    }
    ...
    repn (i, n) {
        repn (j, m) {
            repn (k, n - i) {
                repn (l, m - j) {
                    ans += combi(n, i) * combi(n - i, k) * combi(m, j) * combi(m - j, l) *
                           dp1[i][j] * dp2[k][l];
                }
            }
        }
