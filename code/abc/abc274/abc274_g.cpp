...
int main(void) {
    ...
    rep (i, h) {
        rep (j, w) {
            if (b[i][j] == '.') {
                int x = i, y = j;
                while (!st.count({x, y})) --x;
                int l = lower_bound(all(v), pair<int, int>{x, y}) - v.begin();
                x = i;
                while (!st.count({x, y})) --y;
                int r = lower_bound(all(v), pair<int, int>{x, y}) - v.begin();
                mf.add_edge(l, n + r, Inf);
            }
        }
    }
    co(mf.flow(s, t));
