...
#line 2 "/home/kuhaku/home/github/algo/lib/tree/dynamic_union_find.hpp"
...
int main(void) {
    ...
    cin >> a;
    sort(all(a));
    dynamic_union_find uf;
    rep (i, n) {
        if ((a[i] + 1) % m == a[(i + 1) % n])
            uf.unite(a[i], a[(i + 1) % n]);
    }
    unordered_map<int, ll> mp;
    rep (i, n) mp[uf.root(a[i])] += a[i];
    ll s = 0;
    for (auto [x, y] : mp) chmax(s, y);
    co(accumulate(all(a), 0L) - s);
