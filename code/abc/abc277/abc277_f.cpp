...
int main(void) {
    ...
    vector<pair<int, int>> p(h, {Inf, -Inf});
    rep (i, h) {
        rep (j, w) {
            if (a[i][j]) {
                chmin(p[i].first, a[i][j]);
                chmax(p[i].second, a[i][j]);
            }
        }
    }

    auto it = remove_if(all(p), [](auto x) {
        return x == pair<int, int>{Inf, -Inf};
    });
    p.erase(it, p.end());
    sort(all(p));
    int m = p.size();
    ...
    Graph<void> g(h * w + 10 + h + w);
    // rep (i, h * w) {
    //     g.add_edge(w + i, w + i + 1);
    // }
    int s = w;
    rep (i, h) {
