...
void solve() {
    ...
    rep (i, h) {
        rep (j, w) {
            xs[v[i][j] - 1].emplace_back(i);
            ys[v[i][j] - 1].emplace_back(j);
        }
    }
    vector ans(h - a + 1, vector(w - b + 1, 0));
    rep (z, n) {
        if (xs[z].empty())
            continue;
        int minx = *min_element(all(xs[z]));
        int maxx = *max_element(all(xs[z]));
        int miny = *min_element(all(ys[z]));
        int maxy = *max_element(all(ys[z]));
        rep (i, h - a + 1) {
            rep (j, w - b + 1) {
                if (i <= minx && i + a - 1 >= maxx && j <= miny && j + b - 1 >= maxy) {
                } else {
                    ++ans[i][j];
                }
            }
