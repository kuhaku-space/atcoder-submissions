...
struct Monoid {
    using T = pair<ll, ll>;
    using value_type = T;
    static constexpr T id = T(0, -Inf);
    static constexpr T op(const T &lhs, const T &rhs) {
        return {lhs.first + rhs.first, max(lhs.second, lhs.first + rhs.second)};
    }
...
int main(void) {
    ...
    rep (i, n) {
        int x;
        cin >> x;
        a.emplace_back(x - b, x - b);
    }

    setp(20);
    segment_tree<Monoid> st(a);
    auto f = [&](pair<ll, ll> p) {
        return p.second < 0;
    };
    while (q--) {
        ...
        cin >> c >> x;
        st.set(c - 1, {x - b, x - b});
        int k = st.max_right(0, f);
        if (k < n)
            ++k;
        co((double)st.prod(0, k).first / k + b);
    }
