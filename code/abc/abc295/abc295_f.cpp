...
ll f(vector<int> v, vector<int> x) {
    auto dp = make_vector(0L, x.size() + 1, v.size() + 1, 2);
                ...
                if (j < v.size() && v[j] == k) {
                    ...
                    repr (l, j) {
                        ...
                        if (flag) {
                            f = false;
                            if (x[i] == k)
                                dp[i + 1][l + 1][1] += dp[i][j][1];
                            else if (x[i] > k)
                                dp[i + 1][l + 1][0] += dp[i][j][1];
                            dp[i + 1][l + 1][0] += dp[i][j][0];
                            break;
                        }
                    }
                    if (f) {
    ...
    ll ans = 0;
    repr (i, x.size()) {
        ans += dp[i + 1][v.size()][1] * p;
        ans += dp[i + 1][v.size()][0] * q;
        p += x[i] * q;
        q *= 10;
    }
    return ans;
...
void solve() {
    ...
    cin >> s >> l >> r;
    auto v = to_int(s);
    auto lv = to_int(to_string(l - 1));
    auto rv = to_int(to_string(r));
    co(f(v, rv) - f(v, lv));
}
