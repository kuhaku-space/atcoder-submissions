...
int main(void) {
    ...
    rep (i, n) v[a[i]].emplace_back(i);
    segment_tree<ArgMin> st(n);
    rep (i, n) st.set(i, {a[i], i});
    vector<bool> used(m);
    int l = 0, r = 0;
    vector<int> ans;
    rep (i, m) {
        while (r < n && (c[a[r]] >= 2 || used[a[r]])) {
            --c[a[r]];
            ++r;
        }
        auto [x, y] = st.prod(l, min(r + 1, n));
        ans.emplace_back(x);
        l = y + 1;
        used[x] = true;
        for (int k : v[x]) st.set(k, {Inf, Inf});
    }
    Inc >> ans;
    co(ans);
