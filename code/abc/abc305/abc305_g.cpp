...
string to_str(int x, int n = 6) {
    string t;
    rep (k, n) t += 'a' + (x >> k & 1);
    return t;
}

bool is_ok(string t, const vector<string> &v) {
    bool flag = false;
    for (auto s : v) {
        if (t.size() < s.size())
            continue;
        rep (i, t.size() - s.size() + 1) {
            flag |= t.substr(i, s.size()) == s;
        }
    }
    return !flag;
}
...
int main(void) {
    ...
    rep (bit, 64) {
        string t = to_str(bit);
        int nxt0 = (bit << 1) & 63, nxt1 = nxt0 + 1;
        if (is_ok(to_str(nxt0), v))
            mat[nxt0][bit] += 1;
        if (is_ok(to_str(nxt1), v))
            mat[nxt1][bit] += 1;
    }

    mat = mat.pow(n - 6);
    Matrix<Mint> ans(64, 1);
    rep (bit, 64) {
        string t = to_str(bit);
        if (is_ok(t, v))
            ans[bit][0] = 1;
