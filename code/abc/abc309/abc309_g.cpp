...
int main(void) {
    int n, x;
    cin >> n >> x;
    vector dp(n + 1, vector((1 << (x * 2 - 1)), vector(n + 1, Mint())));
    dp[0][0][0] = 1;
    vector<int> ps(1 << (x * 2 - 1));
    rep (i, 1 << (x * 2 - 1)) ps[i] = ps[i >> 1] + (i & 1);
    rep (i, n) {
        rep (j, (1 << (x * 2 - 1))) {
            rep (k, n) {
                int cr = max(0, n - i - x);
                ...
                int cl = max(0, i - x + 1);
                if (cr == 0) {
                    dp[i + 1][j >> 1][k + 1] += dp[i][j][k] * combi(cr - k, 1);
                    dp[i + 1][j >> 1][k] += dp[i][j][k] * combi(cl - max(0, i - k - p), 1);
                } else {
                    Mint icr = Mint(cr).inv();
                    {
                        Mint y = dp[i][j][k] * combi(cr - k, 1);
                        dp[i + 1][j >> 1][k + 1] += y * (cr - k - 1) * icr;
                        dp[i + 1][(j >> 1) | (1 << (x * 2 - 2))][k] += y * (k + 1) * icr;
                    }
                    {
                        Mint y = dp[i][j][k] * combi(cl - max(0, i - k - p), 1);
                        dp[i + 1][j >> 1][k] += y * (cr - k) * icr;
                        if (k)
                            dp[i + 1][(j >> 1) | (1 << (x * 2 - 2))][k - 1] += y * k * icr;
                    }
                }
            }
        }
    ...
    Mint ans = 0;
    rep (i, 1 << (x * 2 - 1)) {
        ans += accumulate(all(dp[n][i]), Mint());
    }
    co(ans);
