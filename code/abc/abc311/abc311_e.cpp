...
int main(void) {
    ...
    for (auto [x, y] : a) dp[x][y]++;

    rep (i, h) {
        repr (j, w) dp[i][j] += dp[i][j + 1];
    }

    repr (i, h) {
        rep (j, w) dp[i][j] += dp[i + 1][j];
    }

    auto prod = [&dp](int a, int b, int c, int d) {
        return dp[a][b] - dp[a][d] - dp[c][b] + dp[c][d];
    };
    ...
    rep (i, h) {
        rep (j, w) {
            ...
            while (r - l > 1) {
                int m = (l + r) / 2;
                if (i + m > h || j + m > w || prod(i, j, i + m, j + m))
                    r = m;
                else
                    l = m;
            }
