...
int main(void) {
    ...
    vector<int> x(n, -1);
    vector<int> s(n, 1);
    vector<pair<int, int>> y(n, {-1, -1});
    for (auto p : ps) {
        int l = root(x, p.first - 1), r = root(x, p.second - 1);
        x[l] = x.size();
        x[r] = x.size();
        x.emplace_back(-1);
        s.emplace_back(s[l] + s[r]);
        y.emplace_back(l, r);
    }

    vector<Mint> ans(x.size());
    auto dfs = [&](auto self, int idx, Mint m) -> void {
        ans[idx] = m;
        if (y[idx].first == -1)
            return;
        self(self, y[idx].first, m + Mint(s[y[idx].first]) / s[idx]);
        self(self, y[idx].second, m + Mint(s[y[idx].second]) / s[idx]);
    };
    dfs(dfs, x.size() - 1, 0);
    ans.resize(n);
    co(ans);
