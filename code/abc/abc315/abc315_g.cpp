...
#line 3 "/home/kuhaku/atcoder/github/algo/lib/math/floor_sum.hpp"
...
ll solve(ll n, ll a, ll b, ll c, ll x) {
    ll ans = 0;
    repn (i, n) {
        if (x < a * i)
            break;
        ll rest = x - a * i;
        ll l = (rest >= c * n) ? (rest - c * n) / b : 0;
        ll r = rest / b;
        chmin(r, n);
        if (l >= r)
            ans += r * n;
        else
            ans += l * n + floor_sum(r - l, c, b, rest - b * r);
    }
    return ans;
}
...
int main(void) {
    ll n, a, b, c, x;
    cin >> n >> a >> b >> c >> x;
    co(solve(n, a, b, c, x) - solve(n, a, b, c, x - 1));
