...
int main(void) {
    ...
    dists[0] = 0;
    que.emplace(0);
    vector<int> unused_index(n - 1);
    iota(all(unused_index), 1);
    while (!que.empty()) {
        int idx = que.front();
        que.pop();
        vector<int> next_unused_index;
        int ptr = 0;
        for (int to : unused_index) {
            while (ptr < (int)g[idx].size() && g[idx][ptr].to() < to) ++ptr;
            if (ptr < (int)g[idx].size() && g[idx][ptr].to() == to) {
                next_unused_index.emplace_back(to);
            } else {
                dists[to] = dists[idx] + 1;
                que.emplace(to);
            }
        }
        unused_index = move(next_unused_index);
    }
    ...
    int max_d = *max_element(all(dists));
    vector<vector<int>> dist_index(max_d + 1);
    rep (i, n) {
        if (dists[i] != -1)
            dist_index[dists[i]].emplace_back(i);
    }

    vector<Mint> ans(n), sum(n);
    ans[0] = 1;
    rep (i, max_d) {
        for (auto idx : dist_index[i]) sum[i] += ans[idx];
        for (auto idx : dist_index[i + 1]) {
            ans[idx] = sum[i];
            for (auto e : g[idx]) {
                if (dists[e.to()] == dists[idx] - 1)
                    ans[idx] -= ans[e.to()];
            }
        }
    }
