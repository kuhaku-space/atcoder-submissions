...
int main(void) {
    ...
    cin >> x;
    x.emplace(x.begin(), 0);
    vector<pair<int, int>> y(n - 1);
    cin >> y;
    vector dp(h + 1, vector(h + 1, 0L));
    rep (idx, n - 1) {
        int z = x[idx + 1] - x[idx];
        auto [p, f] = y[idx];
        vector cp(h + 1, vector(h + 1, INF));
        rep (i, h + 1) {
            rep (j, h + 1) {
                if (i - z >= 0 && j + z <= h) {
                    chmin(cp[i - z][j + z], dp[i][j]);
                    chmin(cp[min(i - z + f, h)][j + z], dp[i][j] + p);
                    chmin(cp[i - z][max(j + z - f, 0)], dp[i][j] + p);
                }
            }
        }
        dp = cp;
