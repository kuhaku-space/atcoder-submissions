...
int main(void) {
    ...
    rep (_, 2) {
        if (y.first == z.first) {
            vector<ll> v = {x.second, y.second, z.second};
            bool f = false;
            rep (_, 2) {
                f |= v[0] < v[1] && v[1] < v[2];
                swap(v[0], v[2]);
            ...
            if (f) {
                co(abs(x.first - y.first) + abs(x.second - z.second) - 1);
                return 0;
            }
            if (x.first == y.first) {
                co(abs(x.second - y.second) + abs(y.second - z.second) + 3);
            } else {
                co(abs(x.first - y.first) + abs(x.second - y.second) + abs(y.second - z.second) +
                   1);
            }
            return 0;
        }
    ...
    ll ans = abs(x.first - y.first) + abs(x.second - y.second) + abs(y.first - z.first) +
             abs(y.second - z.second) + 3;
    rep (_, 2) {
        vector<ll> v = {x.second, y.second, z.second};
        bool f = false;
        rep (_, 2) {
