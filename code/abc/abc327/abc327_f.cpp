...
int main(void) {
    ...
    cin >> a;
    sort(all(a));
    lazy_segment_tree<Maxi<int>, Add<int>> lst(600100);
    constexpr int b = 200050;
    int idx = 0;
    int ans = 0;
    rep (i, n) {
        lst.apply(a[i].second + b, a[i].second + w + b, 1);
        while (a[idx].first + d <= a[i].first) {
            lst.apply(a[idx].second + b, a[idx].second + w + b, -1);
            ++idx;
        }
        chmax(ans, lst.prod(0, 600100));
    }
    co(ans);
