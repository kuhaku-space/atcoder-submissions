...
template <class T>
std::vector<ll> dijkstra(const Graph<T> &g, int s = 0) {
    struct _node {
        constexpr _node() : _to(), _dist() {}
        constexpr _node(int to, ll dist) : _to(to), _dist(dist) {}
    ...
    std::vector<ll> dists(g.size(), -INF);
    std::priority_queue<_node, std::vector<_node>, std::greater<>> p_que;
    dists[s] = std::numeric_limits<ll>::max();
    p_que.emplace(s, dists[s]);
    while (!p_que.empty()) {
        auto node = p_que.top();
        p_que.pop();
        if (dists[node.to()] > node.dist())
            continue;
        for (auto &e : g[node.to()]) {
            auto [l, d, k, c] = e.weight();
            if (l + c > node.dist())
                continue;
            ll x = -INF;
            if (l + (k - 1) * d + c <= node.dist()) {
                x = l + (k - 1) * d;
            } else {
                x = l + (node.dist() - l - c) / d * d;
            }
            if (chmax(dists[e.to()], x))
                p_que.emplace(e.to(), x);
        }
    }
    return dists;
}
...
int main(void) {
    int n, m;
    cin >> n >> m;
    Graph<tuple<ll, ll, ll, ll>> g(n);
    rep (i, m) {
        int l, d, k, c, a, b;
        cin >> l >> d >> k >> c >> a >> b;
        g.add_edge(b - 1, a - 1, {l, d, k, c});
    }

    auto dist = dijkstra(g, n - 1);
    rep (i, n - 1) {
        if (dist[i] == -INF) {
            co("Unreachable");
        } else {
