#line 1 "a.cpp"
#define PROBLEM ""
#line 1 "/home/kuhaku/home/github/algo/lib/data_structure/flip_set.hpp"
#include <set>
...
int main(void) {
    ...
    auto dfs = [&](auto self, int x) -> void {
        ...
        for (auto e : g[x]) {
            if (used[e.to()])
                continue;
            self(self, e.to());
            if (vertex.size() < k && !vertex.contains(e.to())) {
                edge.flip(e.weight());
                vertex.flip(x);
                vertex.flip(e.to());
            }
    ...
    Yes();
    vector<int> ans;
    for (auto x : edge) ans.emplace_back(x);
    co(ans.size());
    co(ans);
