...
struct Monoid {
    ...
    static constexpr value_type g(const value_type &v, int u) {
        return {v.first + v.second, v.second + a[u]};
    }
};

vector<int> Monoid::a = vector<int>();
...
int main(void) {
    ...
    Monoid::a.resize(n);
    cin >> Monoid::a;
    ReRooting<Monoid, void> rr(g);
    ll ans = std::numeric_limits<ll>::max();
    rep (i, n) chmin(ans, rr[i].first);
    co(ans);
