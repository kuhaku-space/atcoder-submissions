...
int main(void) {
    ...
    unordered_map<ull, Mint> mp1, mp2;
    vector<Mint> v(40);
    rep (i, 40) v[i] = Mint(10).pow(i);
    auto f = [&](auto self, ull x) -> Mint {
        if (mp2.count(x))
            return mp2[x];
        if (x == 0)
            return 1;
        if (x & 1) {
            return mp2[x] = self(self, x / 2) * self(self, x / 2) * 10;
        } else {
            return mp2[x] = self(self, x / 2) * self(self, x / 2);
        }
    };
    auto dfs = [&](auto self, ull x) -> Mint {
        if (x == 1) {
            return n;
        }
        if (mp1.count(x))
            return mp1[x];
        if (x & 1) {
            return mp1[x] = n * f(f, d * (x - 1)) + self(self, x / 2) * (f(f, d * (x / 2)) + 1);
        } else {
            return mp1[x] = self(self, x / 2) * (f(f, d * (x / 2)) + 1);
        }
    };
    co(dfs(dfs, n));
    return 0;
