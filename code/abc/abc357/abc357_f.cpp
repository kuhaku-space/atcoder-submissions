...
struct L {
    Mint a, b;
    constexpr L &operator+=(const L &rhs) {
        a += rhs.a;
        b += rhs.b;
        return *this;
    }
    constexpr L operator+(const L &rhs) const {
        return L(*this) += rhs;
    }
};
struct F {
...
struct G {
    ...
    static constexpr S f(T lhs, S rhs) {
        S res(rhs);
        res.s += lhs.a * rhs.b + rhs.a * lhs.b + lhs.a * lhs.b * rhs.l;
        res.a += lhs.a * rhs.l;
        res.b += lhs.b * rhs.l;
        return res;
    }
...
int main(void) {
    ...
    vector<S> v(n);
    rep (i, n) {
        v[i] = S{a[i] * b[i], 1, a[i], b[i]};
    }
    lazy_segment_tree<F, G> lst(v);
    while (q--) {
        int c;
        cin >> c;
        if (c == 1) {
