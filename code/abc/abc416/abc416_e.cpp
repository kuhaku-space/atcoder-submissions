...
void solve() {
    ...
    cin >> v;
    rep (i, k) {
        g.add_edge(v[i] - 1, n, t);
        g.add_edge(n, v[i] - 1, 0);
    }
    vector d(n, vector(n, INF));
    rep (i, n) {
        auto di = dijkstra(g, i, INF);
        rep (j, n) chmin(d[i][j], di[j]);
    }
    int q;
    cin >> q;
    while (q--) {
        ...
        if (x == 1) {
            ...
            --a, --b;
            g.add_edges(a, b, c);
            auto di = dijkstra(g, a, INF);
            rep (i, n) {
                rep (j, n) chmin(d[i][j], di[i] + di[j]);
            }
        } else if (x == 2) {
            int e;
            cin >> e;
            ...
            g.add_edge(e, n, t);
            g.add_edge(n, e, 0);
            auto di = dijkstra(g, n, INF);
            rep (i, n) {
                rep (j, n) chmin(d[i][j], di[i] + di[j] + t);
            }
        } else {
            ll ans = 0;
            rep (i, n) {
