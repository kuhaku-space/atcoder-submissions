...
int main(void) {
    ...
    vector dp(1 << m, vector(m + 1, vector(2, -Inf)));
    dp[0][m][0] = 0;
    for (auto x : s) {
        auto v = dp;
        rep(i, 1 << m) {
            rep(j, m + 1) {
                rep(k, 2) {
                    ...
                    if (i != (1 << m) - 1 && ii == (1 << m) - 1) {
                        if (j == mp1[x])
                            chmax(dp[ii][mp1[x]][1], v[i][j][k] + z + y + mp2[x]);
                        else
                            chmax(dp[ii][mp1[x]][0], v[i][j][k] + z + mp2[x]);
                    } else {
                        if (j == mp1[x])
                            chmax(dp[ii][mp1[x]][1], v[i][j][k] + y + mp2[x]);
                        else
                            chmax(dp[ii][mp1[x]][0], v[i][j][k] + mp2[x]);
                    }
                }
            }
        }
