...
#line 4 "/home/kuhaku/home/github/algo/lib/fft/ntt_mod.hpp"
...
#line 4 "/home/kuhaku/home/github/algo/lib/math/combination.hpp"
...
int main(void) {
    ...
    FOR (i, a, b + 1) {
        vector<Mint> ndp(n + 1);
        ndp[0] = 1;
        Mint p = combi.finv(i).pow(c);
        FOR (j, c, d + 1) {
            if (j * i > n)
                break;
            ndp[j * i] = p * combi.finv(j);
            p *= combi.finv(i);
        }
        vector<int> v(n + 1);
        rep (i, n + 1) v[i] = ndp[i].val();
        dp = convolution_mod<Mint::mod()>(dp, v);
        dp.resize(n + 1);
    }
    co(dp[n] * combi.fact(n));
