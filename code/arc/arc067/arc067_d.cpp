...
std::pair<std::vector<std::invoke_result_t<F, int, int>>, std::vector<int>> monotone_minima(
    int h, int w, const F &f, const Comp &comp = Comp()) {
    using T = std::invoke_result_t<F, int, int>;
    assert(h >= 0);
    assert(w >= 0);
    std::vector<T> res_value(h);
    std::vector<int> res_idx(h);
    std::stack<std::tuple<int, int, int, int>, std::vector<std::tuple<int, int, int, int>>> stk;
    stk.emplace(0, h - 1, 0, w - 1);
    while (!stk.empty()) {
        auto [i, j, l, r] = stk.top();
        stk.pop();
        int m = (i + j) / 2;
        T min_value = f(m, l);
        int min_idx = l;
        ...
        res_idx[m] = min_idx;
        if (i <= m - 1)
            stk.emplace(i, m - 1, l, min_idx);
        if (m + 1 <= j)
            stk.emplace(m + 1, j, min_idx, r);
    }
...
int main(void) {
    ...
    vector<segment_tree<Max<int>>> seg(m);
    rep (i, m) seg[i] = segment_tree<Max<int>>(b[i]);

    auto f = [&](int l, int r) -> ll {
        if (l > r)
            return 0;
        ll s = -a[r] + a[l];
        rep (i, m) s += seg[i].prod(l, r + 1);
        return s;
    };

    auto ans = monotone_minima<decltype(f), greater<>>(n, n, f).first;
