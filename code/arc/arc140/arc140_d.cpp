...
std::vector<Mint> prod(const std::vector<int> &a, int l, int r) {
    if (r - l == 0)
        return {1};
    if (r - l == 1)
        return {1, a[l]};
    ...
    rep (i, n) {
        if (uf.is_root(i)) {
            if (e[i] == uf.size(i))
                ...
                b.emplace_back(uf.size(i));
        }
    }

    auto ans = prod(b, 0, b.size());
    ce(ans);
    Mint sum = Mint(n).pow(b.size()) * c;
    repn (i, ans.size() - 1) {
        sum += combi.fact(i - 1) * ans[i] * Mint(n).pow(b.size() - i);
    }
    co(sum);
