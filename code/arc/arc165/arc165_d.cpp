...
#line 3 "/home/kuhaku/atcoder/github/algo/lib/graph/scc.hpp"
...
int main(void) {
    ...
    for (auto &[a, b, c, d] : q) cin >> a >> b >> c >> d, --a, --c;
    vector<int> cnt(m);
    vector<int> group(n);
    iota(all(group), 0);
    while (true) {
        Graph<void> g(*max_element(all(group)) + 1);
        rep (i, m) {
            auto [a, b, c, d] = q[i];
            if (a + cnt[i] < b)
                g.add_edge(group[a + cnt[i]], group[c + cnt[i]]);
        }
        auto v = scc(g);
        bool f = true;
        rep (i, m) {
            auto [a, b, c, d] = q[i];
            if (a + cnt[i] < b && v[group[a + cnt[i]]] == v[group[c + cnt[i]]]) {
                f = false;
                while (v[group[a + cnt[i]]] == v[group[c + cnt[i]]]) {
                    ++cnt[i];
                    if (c + cnt[i] >= d) {
                        No();
                        return 0;
