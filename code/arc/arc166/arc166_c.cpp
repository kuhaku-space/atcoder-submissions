#line 1 "a.cpp"
#define PROBLEM ""
#line 2 "/home/kuhaku/atcoder/github/algo/lib/internal/internal_math.hpp"
#include <cstdint>
#include <utility>
...
#line 2 "/home/kuhaku/atcoder/github/algo/lib/internal/internal_type_traits.hpp"
#include <cassert>
#include <numeric>
#include <type_traits>
...
int main(void) {
    ...
    vector<Mint> dp(3000002);
    dp[0] = 1, dp[1] = 1;
    rep (i, 3000000) {
        dp[i + 2] = dp[i + 1] + dp[i];
    }
    vector<Mint> dp2(1000006);
    dp2[0] = 1;
    rep (i, 1000005) {
        dp2[i + 1] = dp2[i] * dp[(i + 1) * 2];
    }
    while (t--) {
        ll h, w;
        cin >> h >> w;
        ll x = min(h, w), y = h + w - x;
        co(dp2[x] * dp2[x] * Mint(dp[x * 2 + 1]).pow(y - x));
    }
