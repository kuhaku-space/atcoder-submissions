...
int main(void) {
    ...
    cin >> s;
    int limit = 27;
    Grid<5> idx(4, limit, limit, limit, k + 1);
    unordered_map<ll, Mint> dp;
    dp[idx.flatten(0, 1, 0, 0, 0)] = 1;
    vector<int> memo = {0, 1, 2, 3, 3, 2, 1, 0};
    vector<int> memo2 = {0, 1, 2, 4};
    rep (i, n) {
        unordered_map<ll, Mint> ndp;
        for (auto [_tmp, val] : dp) {
            ll tmp = _tmp;
            int t = tmp % 4;
            tmp /= 4;
            int x = tmp % limit;
            ...
            if (s[i] == '?') {
                rep (shift, 3) {
                    ...
                    int a = memo[nb];
                    if (a == 0) {
                        ndp[idx.flatten(a, x + 1, y, z, min(k, v + x))] += val;
                    } else if (a == 1) {
                        ndp[idx.flatten(a, x, y + 1, z, min(k, v + y))] += val;
                    } else if (a == 2) {
                        ndp[idx.flatten(a, x, y, z + 1, min(k, v + z))] += val;
                    } else {
                        ndp[idx.flatten(a, x, y, z, min(k, v + w))] += val;
                    }
                }
                continue;
            }
