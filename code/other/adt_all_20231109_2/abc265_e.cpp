...
int main(void) {
    ...
    cin >> q;
    set<pair<ll, ll>> st(all(q));

    vector dp(n + 1, vector(n + 1, vector(n + 1, Mint())));
    dp[0][0][0] = 1;
    rep (i, n) {
        rep (j, n) {
            rep (k, n) {
                ll x = p[0].first * i + p[1].first * j + p[2].first * k;
                ll y = p[0].second * i + p[1].second * j + p[2].second * k;
                if (!st.count({x + p[0].first, y + p[0].second}))
                    dp[i + 1][j][k] += dp[i][j][k];
                if (!st.count({x + p[1].first, y + p[1].second}))
                    dp[i][j + 1][k] += dp[i][j][k];
                if (!st.count({x + p[2].first, y + p[2].second}))
                    dp[i][j][k + 1] += dp[i][j][k];
            }
        }
    }
