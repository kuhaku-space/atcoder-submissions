...
#line 3 "/home/kuhaku/home/github/algo/lib/data_structure/wavelet_matrix.hpp"
...
int main(void) {
    ...
    vector<ll> s(n);
    rep (i, n) s[i] = (a[i].first + a[i].second);
    sort(all(s));
    vector<ll> vx(n), vy(n);
    rep (i, n) vx[i] = a[i].first, vy[i] = a[i].second;
    compressed_wavelet_matrix wmx(vx), wmy(vy);
    sort(all(a), [](auto l, auto r) {
        return l.first < r.first;
    });
    rep (i, n) vx[i] = a[i].first, vy[i] = a[i].second;
    compressed_wavelet_matrix wmz(vy);
    while (q--) {
        ll x, y, z;
        cin >> x >> y >> z;
        if (x + y >= z) {
            auto it = lower_bound(all(vx), x);
            int ans = vx.end() - it;
            co(ans - wmz.range_freq(n - ans, n, y));
        } else {
            auto it = lower_bound(all(s), z);
            int ans = s.end() - it;
            co(ans - wmx.range_freq(n - ans, n, x) - wmy.range_freq(n - ans, n, y));
        }
    }
