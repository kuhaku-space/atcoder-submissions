...
int main(void) {
    ...
    a.emplace_back(0);
    repr (i, n) a[i] += a[i + 1];
    segment_tree<Max<ll>> dp(n + 1, 0);
    segment_tree<Max<ll>> dp2(n + 1, 0);
    dp2.set(0, a[0]);
    repn (i, n) {
        dp.set(i, max(dp.get(i - 1), dp.get(i)));
        if (i - k >= 0)
            dp.set(i, max(dp.get(i), dp2.prod(0, i - k + 1) - a[i]));
        dp2.set(i, dp.get(i) + a[i]);
    }
    co(dp.get(n));
    return 0;
}
