...
struct lazy_segment_tree {
    ...
    void add(int a, int b, U x) {
        ...
        for (int i = 0; i <= logN; ++i) update(l >> i, id), update(r >> i, id);
    }
...
int main(void) {
    ...
    auto op = [](T a, T b) { return T{a.a + b.a, a.len + b.len}; };
    auto mapp = [](S s, T m) { return T{s.b * m.a + s.c * m.len, m.len}; };
    auto comp = [](S a, S b) { return S{a.b * b.b, a.b * b.c + a.c}; };

    lazy_segment_tree lst(n, T{0, 1}, S{1, 0}, op, mapp, comp);
    vector<T> t(n);
    rep(i, n) t[i] = T{a[i], 1};
    lst.build(t);
