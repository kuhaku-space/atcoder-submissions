...
int main(void) {
    ...
    Graph<void> g(n);
    g.input_edges(n - 1);
    vector dp(n, vector(k + 1, vector(4, Mint())));
    auto dfs = [&](auto self, int idx, int par) -> void {
        ...
        for (auto e : g[idx]) {
            ...
            self(self, e.to(), idx);
            auto v = dp[e.to()];
            vector cp(k + 1, vector(4, Mint()));
            rep (i, k + 1) {
                rep (j, k + 1) {
                    if (i + j <= k) {
                        cp[i + j][0] += dp[idx][i][0] * v[j][0];
                        cp[i + j][1] += dp[idx][i][1] * v[j][0];
                        cp[i + j][2] += dp[idx][i][1] * v[j][1];
                        cp[i + j][2] += dp[idx][i][1] * v[j][2];
                        cp[i + j][2] += dp[idx][i][2] * v[j][0];
                        cp[i + j][3] += dp[idx][i][2] * v[j][1];
                        cp[i + j][3] += dp[idx][i][2] * v[j][2];
                        cp[i + j][3] += dp[idx][i][3] * v[j][0];
                    }
                    if (i + j + 1 <= k) {
                        cp[i + j + 1][0] += dp[idx][i][0] * v[j][2];
                        cp[i + j + 1][1] += dp[idx][i][1] * v[j][2];
                        cp[i + j + 1][2] += dp[idx][i][2] * v[j][2];
                        cp[i + j + 1][3] += dp[idx][i][3] * v[j][2];
                        cp[i + j + 1][0] += dp[idx][i][0] * v[j][3];
                        cp[i + j + 1][1] += dp[idx][i][1] * v[j][3];
                        cp[i + j + 1][2] += dp[idx][i][2] * v[j][3];
                        cp[i + j + 1][3] += dp[idx][i][3] * v[j][3];
                    }
                }
            }
            dp[idx] = cp;
    ...
    dfs(dfs, 0, -1);
    co(dp[0][k][0] + dp[0][k - 1][2] + dp[0][k - 1][3]);
