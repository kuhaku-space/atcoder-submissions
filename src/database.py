...
class Database:
    ...
    def insert_record(self, table_name: str, columns_type: dict[str, str], submission_data) -> None:
        ...
        sql_str = (
            f"insert into {table_name} {tuple(columns_type.keys())} values {values_str}"
        )
        self.__cur.execute(sql_str)

    def update_database(
        self, table_name: str, columns_type: dict[str, str], submissions
    ) -> None:
    ...
    def is_saved_submission(self, table_name: str, submission_data) -> bool:
        sql_str = (f"select count(*) from '{table_name}'" f"where id={submission_data['id']}")
        self.__cur.execute(sql_str)
        return self.__cur.fetchone()[0] != 0

    def save_code(self, search_table_name: str, save_table_name: str, columns_type: dict[str, str], submissons) -> None:
        cnt = 0
        for submission_data in submissons:
            if self.is_latest_submission(search_table_name, submission_data) and not self.is_saved_submission(save_table_name, submission_data):
                Submission.write_code(submission_data)
                self.insert_record(save_table_name, columns_type, submission_data)
                cnt += 1
            if cnt >= 100:
                break
